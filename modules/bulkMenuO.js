...
export class BulkMenu extends Application {
	...
	static get defaultOptions() {
		return mergeObject(super.defaultOptions, {
			...
			closeOnSubmit: false,
			tabs: [
				{ navSelector: '.tabs', contentSelector: 'form', inital: 'actors' },
			],
		});
	}
	...
	getData(options = {}) {
		...
		for (let documentType in documentTypes) {
			if (documentType == 'macros') {
				const temp = this.permissionFilterer(
					documentTypes[documentType].documents
				);
	...
	permissionFilterer(inputArray) {
		return inputArray.filter(
			entity =>
				entity.data.permission.default == 3 ||
