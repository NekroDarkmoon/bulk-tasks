    ...
    function getFolderList() {
        return game.folders.reduce(
            (acc, folder) => {
                // @ts-expect-error
                const type =
                    folder.documentCollection?.documentClass.collectionName;
                if (!type) return acc;
...
<section class="bm-dialog-body bm-dialog-body__move">
    <SecondaryNav bind:currentSecondaryTab bind:directory />
    ...
    <footer>
        <button class="bulk-tasks-button" onclick={moveDocs}>
